    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                xn = np.array(paths[i][0][-1])
                yn = np.array(paths[i][1][-1])
                x_goal = goal_state[0]
                y_goal = goal_state[1]
                score = np.sqrt((xn - x_goal) ** 2 + (yn - y_goal) ** 2)
                # score = np.sum(abs(-x_goal * ys + xs * y_goal)/np.sqrt(x_goal**2 + y_goal**2))
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            xs_coll = np.array(paths[j][0][-1])
                            ys_coll = np.array(paths[j][1][-1])
                            score += self._weight / np.sqrt((xn-xs_coll)**2+(yn-ys_coll)**2)
                            # --------------------------------------------------
