...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + sensor_var))

    # 3.2 Compute error state
    x_err = K.dot(y_k - p_check)

    # 3.3 Correct predicted state
    p_hat = p_check + x_err[0:3]
    v_hat = v_check + x_err[3:6]
    Q_phi = Quaternion(axis_angle=angle_normalize(x_err[6:9]))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    q_check =  Quaternion(axis_angle=imu_w.data[k-1]*delta_t).quat_mult_left(q_est[k-1])
    print(p_check[0])
    ...
    Q = np.zeros((6,6))
    Q[:3,:3] = np.eye(3) * delta_t ** 2 * var_imu_f
    Q[3:,3:] = np.eye(3) * delta_t ** 2 * var_imu_w
    ...
    for i in range(gnss.data.shape[0]):
        if gnss.t[i] == imu_f.t[k]:
            y_k_gnss = gnss.data[i]
            is_gnss = True           
            break

    for i in range(lidar.data.shape[0]):
        if lidar.t[i] == imu_f.t[k]:
            y_k_lidar = lidar.data[i]
            is_lidar = True

    p_est[k], v_est[k], q_est[k], p_cov[k] = p_check, v_check, q_check, p_cov_check

    if is_lidar:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar * np.eye(3), p_cov[k], y_k_lidar, p_est[k], v_est[k], q_est[k])
    
    if is_gnss:
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss * np.eye(3), p_cov[k], y_k_gnss, p_est[k], v_est[k], q_est[k])
